...
export function posixScript(cmd, ctx) {
	...
	// which exits 3.
	//
	// pipefail is PROBED IN A SUBSHELL rather than set directly, because it is a
	// bash/zsh extension. Ubuntu's /bin/sh is dash, which rejects it -- and `set`
	// is a POSIX *special* built-in, so a non-interactive shell EXITS when one
	// fails. `set -o pipefail || true` therefore cannot rescue it, and the
	// `2>/dev/null` that used to sit here only hid the message: every shell:"sh"
	// command on Linux died with rc 2 and zero bytes of output, with nothing
	// anywhere to explain why. Inside `( ... )` the failure kills only the
	// subshell and the parent simply reads its status. macOS /bin/sh is bash, so
	// it still gets pipefail.
	//
