 ...
 *
 * File jobs (type="file") ride the same queue on purpose: the per-env
 * serialisation the queue already gives us is what keeps a file edit and an
 * `exec` from touching the same path at the same time.
 */

import { CFG } from "./config.mjs"
import { brokerReq, pushChunk } from "./broker.mjs"
import { runCommand } from "./exec.mjs"
import { runFileJob } from "./fileop.mjs"
...
export async function execWorkerMain() {
	...
	while (!run.stopping) {
		...
		try {
			if (cmd.type === "file") await runFileJob(cmd)
			else await runCommand(cmd)
		} catch (e) {
