...
// The null device, spelled so that it survives Node's path handling.
//
// fs.open() passes its argument through path.toNamespacedPath(), which resolves
// a bare "NUL" against the current directory and then prefixes \\?\. That
// prefix switches CreateFileW to verbatim paths, which turns OFF DOS
// device-name parsing -- so "NUL" became a request for a FILE named NUL inside
// the job's cwd, the open failed with ENOENT, and every command on Windows was
// reported as state:"lost" with runtime_ms 0 before it ever reached spawn().
// \\.\NUL is left untouched by toNamespacedPath and keeps device semantics.
...
async function failCommand(id, error) {
	// The reason is pushed as real OUTPUT, not only as agent_error. The broker
	// currently keeps agent_error out of the tool result, so a precondition
	// failure used to reach the client as state:"lost" with zero bytes and a
	// generic "it died between claiming and spawning, retrying is safe if the
	// command is idempotent" hint. For a permanent condition like a shell that is
	// not installed, following that hint is an infinite loop.
	const bytes = Buffer.from(`gha-mcp: ${error}\n`, "utf8")
	await pushChunk({
		command_id: id,
		start_byte: 0,
	...
	// Also on our own stderr, so it lands in the Actions log as well.
	log(`command ${id} never started: ${error}`)
}
...
export async function runCommand(cmd) {
	...
	//   writeFd  handed to the child as BOTH stdout and stderr. We never write a
	//            single byte through it ourselves.
	...
	//
	// The open mode differs by platform on purpose. POSIX gets "a" for O_APPEND's
	// atomic-append guarantee. Windows must NOT: libuv turns O_APPEND into a handle
	// whose GrantedAccess is FILE_APPEND_DATA *without* FILE_WRITE_DATA, and
	// Cygwin/MSYS bash derives "is this descriptor writable?" from exactly that --
	// so shell:"bash" on Windows silently discarded every byte of stdout and
	// stderr while still reporting the correct exit code. "r+" asks for real write
	// access. It is safe here because the file was created empty on the line above,
	// so position 0 is already EOF, and because the two stdio slots are duplicates
	// of ONE handle and therefore share a single file pointer -- the kernel-level
	// interleave of stdout and stderr survives.
	const writeFd = guarded(() => fs.openSync(rawPath, IS_WIN ? "r+" : "a"))
	...
	try {
		if (cmd.stdin_b64) {
			...
			inFd = fs.openSync(NULL_DEVICE, "r")
		}
	} catch (e) {
