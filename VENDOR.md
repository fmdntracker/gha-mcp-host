...
> **このリポジトリ自体も fork ではありません。** 独自の履歴を持つ新規リポジトリです。
> fork は Actions が既定で無効化されるため、`workflow_dispatch` しか入口を持たない
> 本リポジトリでは「存在意義そのものが最初は動かない」状態になります。
> 以下の移植はすべて **ファイル単位の逐語移植** であり、リポジトリ単位の fork は 1 件もありません。
